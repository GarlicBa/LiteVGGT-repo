...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> Tuple[List[torch.Tensor], int]:
        ...
        images_dino = images
        patch_tokens = self.patch_embed(images_dino)
    ...
    def _process_global_attention(self, tokens, B, S, P, C, global_idx, pos=None,info_map=None):
        ...
        if self.training:
            tokens, m_u = checkpoint(self.global_blocks[global_idx], tokens, pos,global_merging=self.global_merging,info_map = info_map,cal_merge = cal_merge,m_u=self.m_u, use_reentrant=self.use_reentrant)
        else:
            tokens, m_u = self.global_blocks[global_idx](tokens, pos=pos,global_merging = self.global_merging,info_map = info_map,cal_merge = cal_merge,m_u = self.m_u)
